...
local Player = game.Players.LocalPlayer -- VARIABLES
local Character, PrimaryPart
task.spawn(function() if Player.Neutral then game.ReplicatedStorage.Remotes.CommF_:InvokeServer("SetTeam", "Pirates") end end)
...
local FightingStyles = {
    ["Black Leg"] = {
        Type = "Tool",
        Calls = {{"BuyBlackLeg", true}, {"BuyBlackLeg"}}
    },
    ["Electro"] = {
        Type = "Tool",
        Calls = {{"BuyElectro", true}, {"BuyElectro"}}
    },
    ["Fishman Karate"] = {
        Type = "Tool",
        Calls = {{"BuyFishmanKarate", true}, {"BuyFishmanKarate"}}
    },
    ["Dragon Claw"] = {
        Type = "Tool",
        Calls = {{"BlackbeardReward", "DragonClaw", "1"}, {"BlackbeardReward", "DragonClaw", "2"}}
    },
    ["Superhuman"] = {
        Type = "Special",
        Calls = {{"BuySuperhuman", true}, {"BuySuperhuman"}}
    },
    ["Death Step"] = {
        Type = "Tool",
        Calls = {{"BuyDeathStep", true}, {"BuyDeathStep"}}
    },
    ["Electric Claw"] = {
        Type = "Tool",
        Calls = {{"BuyElectricClaw"}}
    },
    ["Sharkman Karate"] = {
        Type = "Tool",
        Calls = {{"BuySharkmanKarate"}}
    },
    ["Dragon Talon"] = {
        Type = "Tool",
        Calls = {{"BuyDragonTalon"}}
    },
    ["Godhuman"] = {
        Type = "Special",
        Calls = {{"BuyGodhuman"}}
...
local Threads = {}
Threads._thread = {}
function SendKey(Keycode, delay) -- MODULES
...
function SaveManager:Load()
    ...
    local success, decoded = pcall(function() return HttpService:JSONDecode(readfile(self.File)) end)
    if success then
...
function Pause()
    for _, v in pairs(Character.PrimaryPart:GetChildren()) do
        if v.Name == "f" and v:IsA("BodyVelocity") then v:Destroy() end
    end
...
function Tween(Inst, Info, Prop)
    ...
    if LocalSettings.FSpeed > 375 then
        Track:Play()
        while Track.PlaybackState == Enum.PlaybackState.Playing or Track.PlaybackState == Enum.PlaybackState.Paused do
            task.wait(5)
            ...
            if Track.PlaybackState == Enum.PlaybackState.Completed then break end
        end
...
function GetNPC(name)
    for _, v in ipairs({ReplicatedStorage.NPCs, workspace.NPCs}) do
        for _, npc in pairs(v:GetChildren()) do
            if npc.Name == name or string.find(npc.Name, name) then return npc end
        end
    end
end
...
function CheckNotification(content)
    ...
    for _, tmp in pairs(Gui:GetChildren()) do
        if tmp and tmp:IsA("TextLabel") and string.find(tmp.Text, content) then return true end
    end
    return false
end
...
function GetSeaBeast()
    for i, v in next, workspace.SeaBeasts:GetChildren() do
        if v.Name == "SeaBeast1" and Player:DistanceFromCharacter(v.HumanoidRootPart.Position) <= 2000 then if v:FindFirstChild("Health") and v.Health.Value > 0 then return v end end
    end
end

function GetTerrorshark()
    for i, v in pairs(workspace.Enemies:GetChildren()) do
        if v.Name == "Terrorshark" and Player:DistanceFromCharacter(v.HumanoidRootPart.Position) <= 2000 then return v end
    end
end
...
function GetFruit()
    for _, v in pairs(workspace:GetChildren()) do
        if (v:IsA("Model") or v:IsA("Tool")) and string.find(v.Name, "Fruit") and v:FindFirstChild("Handle") then return v end
    end
end
    ...
    for _, v in ipairs(workspace.Enemies:GetChildren()) do
        ...
        if hum and root and hum.Health > 0 and v.Name == target.Name then
            ...
            if (root.Position - target.PrimaryPart.Position).Magnitude <= LocalSettings.BringDistance then
                TweenNoDelay(root, TweenInfo.new(Player:DistanceFromCharacter(pos) / LocalSettings.BringSpeed), {
...
function AreEnemiesAlive(name)
    for _, v in ipairs(workspace.Enemies:GetChildren()) do
        local hum = v:FindFirstChild("Humanoid")
        if hum and hum.Health > 0 and string.find(v.Name, name) and Player:DistanceFromCharacter(v.PrimaryPart.Position) <= 20 then return true end
    end
    return false
end
...
function TweenToMobSpawn(target)
    ...
    for _, v in ipairs(Spawn:GetChildren()) do
        if type(target) == "string" and string.find(v.Name, target) and Player:DistanceFromCharacter(v.Position) <= 1000 then
            Tween(PrimaryPart, TweenInfo.new(Player:DistanceFromCharacter(v.Position) / LocalSettings.FSpeed, Enum.EasingStyle.Linear), {
                CFrame = v.CFrame * CFrame.new(0, 15, 0)
            })
...
function GetMobSpawn(target)
    local Spawn = workspace._WorldOrigin.EnemySpawns
    for _, v in ipairs(Spawn:GetChildren()) do
        if type(target) == "string" and string.find(v.Name, target) then return v end
    end
end
...
function FollowEnemy(enemy, pos)
    ...
    if enemy.Parent and hum.Health > 0 then
        ...
        Tween(PrimaryPart, TweenInfo.new(Player:DistanceFromCharacter(root.Position) / LocalSettings.FSpeed, Enum.EasingStyle.Linear), {
...
function GetCakeBoss()
    for _, x in ipairs({game.ReplicatedStorage, workspace.Enemies}) do
        for i, enemy in pairs(x:GetChildren()) do
            if enemy:IsA("Model") and (enemy.Name == "Cake Prince" or enemy.Name == "Dough King") then return enemy end
        end
    end
end

function HasFightingStyle(styleName)
    return (Player.Backpack:FindFirstChild(styleName) and Player.Backpack:FindFirstChild(styleName).Level.Value >= 400) or (Character:FindFirstChild(styleName) and Character:FindFirstChild(styleName).Level.Value >= 400)
end
...
function HandleEvent(event)
    ...
    elseif event == "SeaBeast" then
        ...
        if Seabeast then
            ...
            task.wait(.5)
            Tween(PrimaryPart, TweenInfo.new(Player:DistanceFromCharacter(Seabeast:GetPivot().Position) / LocalSettings.FSpeed, Enum.EasingStyle.Linear), {
                CFrame = SeaBeast:GetPivot() * CFrame.new(randX, 15, randZ)
            })
...
local function HandleSeaProgression()
    ...
    if LocalSettings.CurrentPlace == "First-Seas" and level >= 700 then -- Go to Second Sea
        States.Busy = true
        ...
        Tween(PrimaryPart, TweenInfo.new(Player:DistanceFromCharacter(a.Position) / LocalSettings.FSpeed, Enum.EasingStyle.Linear), {
            CFrame = a
        })
        ...
        repeat
            ...
            if not enemy then TweenToMobSpawn("Ice Admiral") end
        until enemy
    ...
    if LocalSettings.CurrentPlace == "Second-Seas" and level >= 1500 then -- Go to Third Sea
...
function GetCFrameQuest()
    for _, x in ipairs({workspace.NPCs, ReplicatedStorage.NPCs}) do
        for i, v in next, x:GetChildren() do
            if v.Name == require(ReplicatedStorage.GuideModule).Data.LastClosestNPC then return v end
        end
    end
end
...
function GetQuestUnlocked()
    ...
    for i, v in next, require(ReplicatedStorage.Quests) do
        for ID, v1 in next, v do
            local LvReq = v1.LevelReq
            for i1, v2 in next, v1.Task do
                if i == NameQuest then if LvReq <= Player.Data.Level.Value and v2 > 1 and v1.Name ~= "Town Raid" then table.insert(TotalQuestUnlocked, v) end end
            end
        end
    end
    ...
    if Player.PlayerGui.Main.Quest.Visible == false and GetLastQuest() and GetLastQuest() == Name and #GetQuestUnlocked() >= 2 then
        for i, v in next, require(ReplicatedStorage.Quests) do
            for ID, v1 in next, v do
                local LvReq = v1.LevelReq
                for i1, v2 in next, v1.Task do
                    if i1 ~= Name and i == NameQuest then if LvReq <= Player.Data.Level.Value and v2 >= 5 and v1.Name ~= "Town Raid" then return i1, i, ID end end
                end
            end
        end
    end
...
function tele(cframe)
    ...
    if LocalSettings.CurrentPlace == "First-Seas" then
        ...
        elseif (lv >= 450 and lv <= 475) or Name == "God's Guard" then
            local args = {"requestEntrance", vector.create(-4607.8232421875, 874.3909912109375, -1667.5570068359375)}
            game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer(unpack(args))
        elseif (lv >= 475 and lv <= 525) or Name == "Shanda" then
            local args = {"requestEntrance", vector.create(-7894.6181640625, 5547.14208984375, -380.2909851074219)}
            game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer(unpack(args))
        end
    end
    ...
    if LocalSettings.CurrentPlace == "Second-Seas" then
        if lv >= 1250 and level <= 1350 then
            local args = {"requestEntrance", vector.create(923.2130126953125, 126.97599792480469, 32852.83203125)}
            CommF_:InvokeServer(unpack(args))
        end
    end

    Tween(PrimaryPart, TweenInfo.new(Player:DistanceFromCharacter(cframe.Position) / LocalSettings.FSpeed, Enum.EasingStyle.Linear), {
        CFrame = cframe
    })
end
...
function InvokeStyleCalls(styleData)
    for _, args in ipairs(styleData.Calls) do
        ...
        task.wait(0.25)
        if ok then print("Tried:", unpack(args), "->", result) end
    end
end
...
function CheckFightingStyle(styleName)
    ...
    if HasFightingStyle(styleName) then return true end
...
function IsRaiding(x)
    for i, v in next, workspace._WorldOrigin.Locations:GetChildren() do
        if v.Name == "Island 1" and Player:DistanceFromCharacter(x.Position, v.Position) <= 10000 then return true end
    end
    return false
end
...
function CheckItem(item)
    for i, v in next, Player.Backpack:GetChildren() do
        if v:IsA('Tool') and (tostring(v) == item or v.Name == item or string.find(v.Name, item)) then return v end
    end
        ...
        if workspace.Map.Jungle.QuestPlates.Door.CanCollide then
            ...
            if workspace.Map.Desert.Burn.Part.CanCollide then
                if not CheckItem("Torch") then
                    ...
                    EquipItem(Item)
                    Tween(PrimaryPart, TweenInfo.new(Player:DistanceFromCharacter(door:GetPivot().Position) / LocalSettings.FSpeed, Enum.EasingStyle.Linear), {
                ...
                if Progress ~= 0 and Progress ~= 1 then
                    if not CheckItem("Cup") then
                        ...
                        Tween(PrimaryPart, TweenInfo.new(Player:DistanceFromCharacter(Cup:GetPivot().Position) / LocalSettings.FSpeed, Enum.EasingStyle.Linear), {
                            CFrame = Cup:GetPivot()
                        })
                    else
...
function GetBartiloPlates()
    ...
    local Select
    table.sort(Plates, function(i, v) return tonumber(i.Name:match("%d+$")) < tonumber(v.Name:match("%d+$")) end)
...
task.spawn(function()
    if game.PlaceId == 7449423635 then -- SPAWN
...
task.spawn(function()
    ...
    for _, v in next, {game.ReplicatedStorage.Util, game.ReplicatedStorage.Common, game.ReplicatedStorage.Remotes, game.ReplicatedStorage.Assets, game.ReplicatedStorage.FX} do
        ...
        v.ChildAdded:Connect(function(n) if n:IsA("RemoteEvent") and n:GetAttribute("Id") then remote, idremote = n, n:GetAttribute("Id") end end)
    end
    ...
    while task.wait(0.05) do
            ...
            for _, v in ipairs(x and x:GetChildren() or {}) do
                ...
                if v ~= char and hrp and hum and hum.Health > 0 and (hrp.Position - root.Position).Magnitude <= LocalSettings.AttackDistance then
                    for _, _v in ipairs(v:GetChildren()) do
                        if _v:IsA("BasePart") and (hrp.Position - root.Position).Magnitude <= LocalSettings.AttackDistance then parts[#parts + 1] = {v, _v} end
                    end
                end
            end
        end
        ...
        if #parts > 0 and tool and (tool:GetAttribute("WeaponType") == "Melee" or tool:GetAttribute("WeaponType") == "Sword" or tool:GetAttribute("WeaponType") == "Demon Fruit") then
            pcall(function()
                ...
                cloneref(remote):FireServer(string.gsub("RE/RegisterHit", ".", function(c) return string.char(bit32.bxor(string.byte(c), math.floor(workspace:GetServerTimeNow() / 10 % 10) + 1)) end), bit32.bxor(idremote + 909090, game.ReplicatedStorage.Modules.Net.seed:InvokeServer() * 2), head, parts)
            end)
        end
    end
end)
...
AddFunction("autoChest", function()
    ...
    while task.wait(.2) do
        ...
        for _, v in ipairs(map:GetChildren()) do
            if v:IsA("Model") then table.insert(route, v) end
        end
        ...
        if #route > 0 then
            ...
            if target and target:IsA("Model") then
                Tween(PrimaryPart, TweenInfo.new(Player:DistanceFromCharacter(target:GetPivot().Position) / LocalSettings.FSpeed, Enum.EasingStyle.Linear), {
                    CFrame = target:GetPivot()
                })
            end
...
AddFunction("autoBone", function()
    while task.wait() do
        local Character = Player.Character or Player.CharacterAdded:Wait()
        local PrimaryPart = Character.PrimaryPart or Character:FindFirstChild("HumanoidRootPart")
        local enemyList = {"Reborn Skeleton", "Living Zombie", "Demonic Soul", "Posessed Mummy",}
...
AddFunction("autoKatakuri", function()
    while task.wait() do
        local Character = Player.Character or Player.CharacterAdded:Wait()
        local PrimaryPart = Character.PrimaryPart or Character:FindFirstChild("HumanoidRootPart")
        local enemyList = {"Cookie Crafter", "Cake Guard", "Baking Staff", "Head Baker",}
...
AddFunction("activateV4", function()
    while task.wait(1) do
        if game.Players.LocalPlayer.Character:FindFirstChild("RaceEnergy") and game.Players.LocalPlayer.Character.RaceEnergy.Value >= 1 and not game.Players.LocalPlayer.Character.RaceTransformed.Value then game:GetService("ReplicatedStorage").Events.ActivateRaceV4:Fire() end
    end
end)

AddFunction("teleFruit", function()
    local fruit = GetFruit()
    if fruit then Character:PivotTo(fruit:GetPivot()) end
end)
    ...
    while task.wait() do
        ...
        if not Player.PlayerGui.Main.Quest.Visible then
            ...
            if Mob then
                ...
                if e then Character:BreakJoints() end
            end
        end
    end
end)
...
AddFunction("checkFruit", function()
    ...
    if fruit and not CheckInventory(fruit:GetAttribute("OriginalName")) then ReplicatedStorage.Remotes.CommF_:InvokeServer("StoreFruit", fruit:GetAttribute("OriginalName"), fruit) end
end)
    ...
    while task.wait(1) do
        if Player.PlayerGui.Main.TopHUDList.RaidTimer.Visible then
            ...
            if Mob then
                ...
                Tween(PrimaryPart, TweenInfo.new(Player:DistanceFromCharacter(targetCFrame.Position) / LocalSettings.FSpeed, Enum.EasingStyle.Linear), {
                    CFrame = targetCFrame
                })
            end
        end
