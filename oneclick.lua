...
local Status = Win:Tab("OneClick", "mouse-pointer-click")
	...
	for _, p in next, {workspace.NPCs, game.ReplicatedStorage.NPCs} do
		for i,v in next, p:GetChildren() do
			if v.Name == "Set Home Point" then
				table.insert(HomePointNPC, v.HumanoidRootPart)
			end
...
function strictCompare(s, pattern)
	if s == pattern or string.find(s, pattern) then
		return true
	end
	return false
end
...
function TweenToMobSpawn(target)
	local Spawn = ReplicatedStorage.FortBuilderReplicatedSpawnPositionsFolder
...
local st = Status:Button({Title = "Status", SubTitle = "---", Callback = function(s) end})
...
AddFunction("executePromoCode", function()
	st:ChangeText("Executing code")
	local Codes = {"JCWK","ADMINFIGHT","GIFTING_HOURS","1LOSTADMIN","NOMOREHACK","BANEXPLOIT","WildDares","BossBuild","GetPranked","EARN_FRUITS","SUB2GAMERROBOT_EXP1","Sub2UncleKizaru","FIGHT4FRUIT","kittgaming","TRIPLEABUSE","Sub2CaptainMaui","Sub2Fer999","Enyu_is_Pro","Magicbus","Starcodeheo","Bluxxy","krazydares","24NOADMIN","ADMIN_TROLL","NEWTROLL","SECRET_ADMIN","staffbattle","TheGreatAce","StrawHatMaine","TantaiGaming","Sub2Daigrock","Axiore","Sub2NoobMaster123"}
            ...
            if workspace.Map.Jungle.QuestPlates.Door.CanCollide then
                ...
                if workspace.Map.Desert.Burn.Part.CanCollide then
                    ...
                    elseif Progress == 0 then
                        ...
                        if Mob then
                            st:ChangeText("Attacking Mob Leader")
							pcall(function()
								repeat task.wait()
									FollowEnemy(Mob, PrimaryPart.Position - Vector3.new(0,15,0))
...
AddFunction("healthCheck", function()
	while task.wait() do
...
AddFunction("oneClick", function()
	...
	while task.wait() do
		...
		if fruit and not CheckInventory(fruit:GetAttribute("OriginalName")) then
			local tries = 0
			st:ChangeText("Getting fruit")
			CloseThread("autoLevel")
			repeat task.wait(1)
				Character:PivotTo(fruit.Handle:GetPivot())
				Character.Humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
				tries += 1
			until Player.Backpack:FindFirstChild(fruit.Name) or Character:FindFirstChild(fruit.Name) or tries >= 10
			ReplicatedStorage.Remotes.CommF_:InvokeServer("StoreFruit", fruit:GetAttribute("OriginalName"), fruit)
		end
		...
		if Player.Data.Level.Value <= 2800 then
			...
			if LocalSettings.CurrentPlace == "Second-Seas" then
				...
				if Player.Data.Level.Value >= 850 then
					CloseThread("autoLevel")
					st:ChangeText("Completing Bartilo Quest")
					bartiloQuest()
...
task.spawn(function()
	...
	Buso()
	StartFunction("executePromoCode")
